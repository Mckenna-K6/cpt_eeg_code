...
def instructions_and_start_Practice(test_type):
    screen.fill(WHITE)
    instruction_text = instruction_font.render("Press SPACE for every letter besides for 'S', and press 'S' when you see the letter 'S'", True, BLACK)
    screen.blit(instruction_text, (WIDTH // 2 - instruction_text.get_width() // 2, HEIGHT // 3))
    start_button = Button(WIDTH // 2 - 100, HEIGHT // 2, 200, 75, "Start", PURPLE, (100, 100, 255), lambda: continuous_performance_test_AX(test_type))
    running = True
    while running:
        screen.fill(WHITE)
        screen.blit(instruction_text, (WIDTH // 2 - instruction_text.get_width() // 2, HEIGHT // 3))
...
def instructions_and_start_1(test_type):
    screen.fill(WHITE)
    instruction_text = instruction_font.render("Press SPACE for all letters except 'X'. Press 'X' when you see 'X'.", True, BLACK)
    screen.blit(instruction_text, (WIDTH // 2 - instruction_text.get_width() // 2, HEIGHT // 3))
    start_button = Button(WIDTH // 2 - 100, HEIGHT // 2, 200, 75, "Start", PURPLE, (100, 100, 255), lambda: continuous_performance_test_AX(test_type))
    running = True
    while running:
...
def instructions_and_start_2(test_type):
    screen.fill(WHITE)
    instruction_text = instruction_font.render("Press SPACE for every letter except the one that follows 'V'. Press 'C' for the letter after 'V'", True, BLACK)
    screen.blit(instruction_text, (WIDTH // 2 - instruction_text.get_width() // 2, HEIGHT // 3))
    start_button = Button(WIDTH // 2 - 100, HEIGHT // 2, 200, 75, "Start", PURPLE, (100, 100, 255), lambda: continuous_performance_test_AX(test_type))
    running = True
    while running:
...
def continuous_performance_test_AX(test_type):
    ...
    num_targets = total_correct + omission_errors
    num_nontargets = total_correct + commission_errors  # assuming 1 response per trial
    hit_rate = total_correct / num_targets if num_targets > 0 else 0
    false_alarm_rate = commission_errors / num_nontargets if num_nontargets > 0 else 0
...
# Defince each test function
def test_1():
    instructions_and_start_Practice(test_type=1)
    continuous_performance_test_AX(test_type=1)


def test_2():
    instructions_and_start_1(test_type=2)
    continuous_performance_test_AX(test_type=2)


def test_3():
    instructions_and_start_2(test_type=3)
